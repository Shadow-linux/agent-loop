...
| “这个字段/算法/API 要调整” | 功能回流或新 feature 判定 | 根据最近 15 天 feature 和代码证据，推荐一个处理流 |
...
| 步骤 | Agent 会做什么 |
|---|---|
| 查最近 feature | 默认检查最近 15 天的 active / paused / closed feature |
| 匹配证据 | 对比 spec、tasks、tests、notes、代码路径、API、模型、UI、测试失败 |
| 给候选表 | 列出可能归属哪个 feature、匹配强度、证据 |
| 推荐处理流 | 回流旧 feature、创建 linked 新 feature、维护修复、或先调查 |
