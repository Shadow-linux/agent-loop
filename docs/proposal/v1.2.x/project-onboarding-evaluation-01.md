...
当前实现已调整：新 Deep Scan 默认 Expanded。下表是旧草案的分层思路，只保留为背景；Compact / Standard 不再由项目大小自动触发，只在人类明确要求更少文件、合并文件，或维护已有 Compact/Standard onboarding-db 时使用。

| 模式 | 文件策略 | 适用项目 | 解决的核心矛盾 |
|---|---|---|---|
| **Expanded** | 分类目录 + 必要主题独立拆分 | 新 Deep Scan 默认 | 不丢失关键细节，支持长期维护 |
| **Standard** | 较少分类文档 | 人类明确要求折中文件数量，或维护已有 Standard | 文件数和可读性折中 |
| **Compact** | 少量合并文档 | 人类明确要求 Compact / 少文件 / 合并文件，或维护已有 Compact | 减少物理文件，不减少理解维度 |
