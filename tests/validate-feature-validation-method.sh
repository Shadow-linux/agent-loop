...
assert_contains "$method" "初始 Feature/显式 Bug 仍按适用规则保留真实 TDD RED"
assert_contains "$method" "Review Repair/clearly eligible Lightweight Change 先修复后 fresh targeted verification"
assert_contains "$method" "Existing Test Obligation 不得降级为 advisory"
assert_contains "$method" "Additional Regression Test Advisory 只有在当前可靠证明和既有义务都完成后才不阻塞"
