...
assert_file_exists scripts/feature_authority_support.py
...
assert_contains references/runtime.md 'when it matches the already resolved Requirement source filename, Feature Context reuses that confined resolved source'
assert_contains references/design.md 'facts, not workflow authorization'
assert_contains references/design.md 'Feature Context resolves that basename through the Requirement README'
...
assert_contains references/validation-scenarios.md 'Nested Requirement Product Definition Basename Reuses The Resolved Source'
...
assert_not_contains templates/spec.md 'Authority Type: feature | bug | human'
