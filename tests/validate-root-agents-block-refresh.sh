...
current_block_version="1.2.4-20260629"
...
assert_contains "templates/root-AGENTS.md" "When refreshing, compare each block against the current template by \`section\` and full \`block-version\`, e.g. \`$current_block_version\`. Bare versions like \`1.2.4\` are stale."
...
assert_contains "references/project-guidance.md" "The script validates section presence, marker integrity, file-level managed version, per-section \`block-version\`, unexpected managed sections, and local \`source\` paths."
...
assert_contains "references/workflow-checklists.md" 'Do not write bare `block-version:<agent-loop-version>` values; copy the full template block revision such as `block-version:1.2.4-20260629`.'
