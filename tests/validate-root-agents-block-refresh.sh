...
current_block_version="1.5.8-20260826.1"
...
if [ "$managed_count" -ne 13 ]; then
  printf 'FAIL: root AGENTS must contain exactly 13 managed blocks; found %s\n' "$managed_count" >&2
  exit 1
fi
...
if ! [ "$bootstrap_line" -lt "$ownership_line" ] ||
  ...
  printf 'FAIL: root AGENTS startup order must be Bootstrap -> Agent Ownership -> Message Intent -> Workflow Gateway Map\n' >&2
  exit 1
fi
...
assert_contains "templates/root-AGENTS.md" "Own the project outcome, not only the workflow: inspect all safely available code, Git, tests, documentation, environment, and memory evidence before asking the human, then continue through the authorized scope until verified completion or a concrete Human Gate."
assert_contains "references/project-guidance.md" "Agent Ownership: agents own the project outcome as well as the loop, inspect safely available evidence before asking"
...
assert_not_contains "templates/root-AGENTS.md" "When refreshing, compare each block against the current template by \`section\` and full \`block-version\`, e.g. \`$current_block_version\`. Bare versions like \`1.5.6\` are stale."
assert_contains "templates/root-AGENTS.md" "Before normal submit/readiness actions, inspect the intended diff, fresh verification, Review, Drift Check, project-memory status, branch/release constraints, and unrelated work."
assert_contains "templates/root-AGENTS.md" 'For an explicit commit or commit-and-push request, show one lightweight confirmation of the entire-worktree summary and proposed commit message; Commit never triggers tests merely for Git, then use `git add -A`; plain commit excludes Push, while commit-and-push authorizes both in order after confirmation.'
assert_contains "templates/root-AGENTS.md" 'Disclose predictable Push-triggered CI without duplicating the same full run locally; manual CI reruns need fresh authority unless the same Human decision supplied an explicitly bounded retry count and condition that still applies, and declined required validation keeps Release/readiness blocked.'
...
while IFS= read -r reference; do
  if [ ! -f "$root/$reference" ]; then
    printf 'FAIL: Workflow Gateway Map points to missing skill reference: %s\n' "$reference" >&2
    exit 1
  fi
done < <(
  awk '
...
assert_contains "references/project-guidance.md" 'Treat bare skill-version-only block revisions such as `block-version:1.5.8` as stale because they cannot distinguish same-version template revisions.'
...
assert_contains "references/project-guidance.md" "Do not require a separate Managed Block Rule or Agent Loop Guidance Version prose section in target root \`AGENTS.md\`; managed block maintenance rules live in this reference and refresh tooling."
assert_contains "references/project-guidance.md" 'It returns `STRUCTURAL_CURRENT / 0`, `STRUCTURAL_CHANGED / 0`, or `STRUCTURAL_INVALID / 1`.'
...
assert_contains "references/workflow-checklists.md" 'Do not write bare `block-version:<agent-loop-version>` values; copy the full template block revision such as `block-version:1.5.8-20260826.1`.'
...
assert_contains "references/validation-scenarios.md" "Root Workflow Gateway Map Routes To Detailed References"
...
assert_contains "Usage.md" "这是普通 Submit/readiness 检查。Agent 会复核 feature 文档、requirement 记录、代码 diff、验证证据、drift、project memory、root/directory guidance 影响和 unrelated changes，但不会执行 Git mutation。"
