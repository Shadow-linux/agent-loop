...
assert_contains "references/runtime.md" 'If `project.md` declares a Decisions index, list decision files before Decision & Design or Feature Spec.'
...
assert_contains "references/stage-guides.md" "Confirmed Product Definition -> Requirement lifecycle -> Design Readiness Check -> Decision & Design If Needed -> Feature Mapping -> Feature Spec with Product Slice"
...
assert_contains "references/product-brief.md" "If an existing Feature Product Brief conflicts with the Effective Product Definition, stop for Requirement conflict/recovery"
assert_contains "references/workflow-checklists.md" "Propose missing Applicable Decision references for human confirmation; do not create a duplicate ADR because a link is missing."
...
assert_contains "README.md" "→ Design Readiness / Decision & Design If Needed"
...
assert_contains "references/validation-scenarios.md" 'recognize `Requirement Product Definition -> Design Readiness Check -> Decision & Design If Needed -> Feature Mapping -> Feature Spec Product Slice`'
