...
assert_contains "$reference" 'Recommendation and adoption do not authorize branch creation, switching, merge, deletion, push, tag, release, publish, or seal.'
assert_contains "$reference" '| Branch Action Gate | creation or switching of one exact development branch |'
assert_contains "$reference" '| Tag Gate | creation of one exact tag at one exact commit |'
assert_contains "$reference" '| Push Gate | push of exact refs to one exact remote |'
assert_contains "$reference" '| Release Gate | creation or finalization of one exact release record for one version |'
assert_contains "$reference" '| Publish Gate | publication of one exact package or artifact to one exact channel or destination |'
assert_contains "$reference" '| Seal Gate | transition of one exact version to `released / sealed` after required release evidence succeeds |'
assert_contains "$reference" 'A Batch Human Review may present several action gates together, but each row remains an independent decision.'
assert_contains "$reference" 'Approving all displayed rows authorizes only those exact named rows, in the displayed order and subject to their stated preconditions.'
assert_contains "$reference" '`release-candidate`: intended scope is aggregated and awaiting verification, Review, Drift Check, and the applicable independent Tag, Push, Release, Publish, and Seal Gates.'
assert_not_contains "$reference" '| Release Gate | exact tag, push, release, publish, and sealed transition |'
...
assert_contains references/submit-and-integrate.md 'A Batch Human Review may present Tag, Push, Release, Publish, and Seal together, but every action remains an independent decision row'
assert_contains references/submit-and-integrate.md 'Card execution approval cannot become commit, push, PR, merge, tag, release, publish, or seal approval'
assert_contains references/submit-and-integrate.md 'Strategy adoption and plan approval are context only. They never authorize branch creation, switching, merge, deletion, push, tag, release, publish, or seal.'
assert_contains references/submit-and-integrate.md '-> Tag Gate when required'
assert_contains references/submit-and-integrate.md '-> Publish Gate when required'
assert_contains references/submit-and-integrate.md '-> Seal Gate'
assert_contains references/submit-and-integrate.md 'Unresolved observed memory conflicts block tag, push, release, publish, seal, and Source cleanup'
assert_contains references/memory-reconciliation.md 'commit, push, PR, merge, tag, release, publish, seal, branch deletion, or cleanup'
assert_contains references/external-skill-adapters.md 'merge, tag, release, publish, seal, close the Feature'
assert_contains references/lightweight-change-lane.md 'Commit, push, PR, merge, tag, release, publish, seal, deployment'
assert_contains references/stage-guides.md 'merge, release, publish, seal, or claim submission readiness without human confirmation'
...
RUBY
assert_contains templates/root-AGENTS.md 'Submit, commit, push, PR, merge, tag, release, publish, seal, pause, close, and cleanup remain independent Human Gates.'
assert_contains templates/root-AGENTS.md 'Submit, commit, PR, merge, release, publish, seal, pause, close, or cleanup is requested'
assert_contains templates/root-AGENTS.md 'Git And Lifecycle Gate: branch mutation, commit, push, PR, merge, tag, release, publish, seal, pause, close, Full Memory Audit / Recovery Apply/Restore, or cleanup.'
assert_contains SKILL.md 'submit, commit, PR, merge, release, publish, seal, pause, or close is requested'
assert_contains references/runtime.md 'submit, commit, PR, merge, release, publish, seal, pause, or close is requested'
...
assert_contains references/human-review-summary.md '| Push Gate / Push | exact refs + exact remote |'
assert_contains references/human-review-summary.md '| Release Gate / Release | exact version + release record |'
assert_contains references/human-review-summary.md '| Publish Gate / Publish | exact package/artifact + channel/destination |'
assert_contains references/human-review-summary.md '| Seal Gate / Seal | exact version -> `released / sealed` |'
assert_contains references/human-review-summary.md '| Requested Authorization | prepare / create / switch / commit / push / merge / delete / tag / release / publish / seal |'
assert_contains references/human-review-summary.md '| Explicitly Not Authorized | commit, push, tag, release, publish, seal, merge, branch delete, Source cleanup |'
assert_contains references/human-review-summary.md 'Each row is an independent Human Gate decision.'
assert_contains references/runtime.md 'Tag, Push, Release, Publish, and Seal remain independent action gates.'
assert_contains references/design.md 'Tag, Push, Release, Publish, and Seal remain independent action gates.'
assert_contains references/stage-guides.md 'A batch review may present multiple exact Git/release actions together, but each action keeps an independent decision and precondition.'
assert_contains references/workflow-checklists.md 'When using one Batch Human Review, keep each requested Git/release action as an independent decision row with exact scope and preconditions.'
assert_contains references/workflow-checklists.md 'Do not treat recommendation, adoption, plan acceptance, or auto mode as permission to create, switch, merge, delete, push, tag, release, publish, or seal.'
assert_contains references/workflow-checklists.md 'Treat each create/switch/merge/delete/push/tag/release/publish/seal action as separately Human-gated'
assert_contains references/workflow-checklists.md 'next independent Memory Commit / Tag / Push / Release / Publish / Seal / Cleanup Human Gate'
...
assert_contains references/validation-scenarios.md 'Batch Release Review Keeps Independent Decisions'
...
assert_contains README.md 'Tag, Push, Release, Publish, and Seal remain independent Human Gate decisions'
assert_contains README.md 'branch mutation, commit, push, PR, merge, tag, release, publish, and seal'
...
assert_contains Usage.md '同一张 Batch Human Review 可以一次展示这些动作，但每个动作仍须独立决定'
assert_contains Usage.md 'Batch Human Review<br/>Tag / Push / Release / Publish / Seal<br/>各自独立决定'
assert_contains CHANGELOG.md 'Human-Guided Branch Management'
assert_contains CHANGELOG.md 'Tag、Push、Release、Publish、Seal'
assert_contains docs/proposal/v1.4.x/branch-management-strategy.md '| Tag Gate | 创建一个指向精确 commit 的精确 tag |'
assert_contains docs/proposal/v1.4.x/branch-management-strategy.md 'Batch Human Review<br/>Tag / Push / Release / Publish / Seal<br/>各自独立决定'
assert_contains docs/proposal/v1.4.x/branch-management-strategy-implementation-plan.md '2026-08-12 发布前审计维护补充'
