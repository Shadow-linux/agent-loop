...
assert_contains references/runtime.md 'Feature Spec enters Brainstorm / Clarify only when concrete feature-local scope, acceptance, or implementation-boundary uncertainty remains after loading the resolved Feature Authority, accepted Feature boundary, applicable Product Slice, and applicable ADRs.'
assert_contains references/design.md 'A clear Feature with resolved Feature Authority, an accepted Feature boundary, applicable Product Slice/accepted ADRs, explicit scope, and measurable acceptance skips Brainstorm / Clarify.'
...
assert_contains references/stage-guides.md 'When the accepted Feature boundary, applicable Product Slice/ADR Design Slices, scope, exclusions, and acceptance are already clear, skip Brainstorm / Clarify and proceed directly with Feature Spec.'
...
assert_contains Usage.md 'Feature 已经有明确的 Feature Authority、已接受 Feature 边界、适用 Product Slice / ADR、范围、排除项和验收时，Agent 不会为了形式调用 brainstorming'
