...
## Checker Applicability And Fact Output

Run the core-flow coverage Checker only against a requested local onboarding root. Applicability is decided before completeness: a directory with no recognizable current Evidence-Graph + DDD Onboarding artifacts returns `NOT_APPLICABLE / 0`; once any canonical Evidence Graph, Onboarding Spec, Tasks, coverage/review artifact, or flow document makes the scope recognizable, missing companion artifacts are objective `CHANGED / 0` findings rather than proof that another domain applies. A result never accepts an Onboarding Spec, grants the Full Execution Gate, or declares the material newcomer-ready.

For an applicable scope, `CURRENT / 0` means the mechanically declared files, flow/slice IDs, locators, evidence references, diagram definitions, source/render pairs, digests, and recorded status values resolve. `CHANGED / 0` lists deterministic per-flow findings for missing or stale facts. Preserve every declared `covered`, `PASS`, deferred, status, and score value as a fact; self-declared `PASS` is not semantic proof. Equivalent non-English wording, explanation quality, call/data direction meaning, business-terminal completeness, and newcomer usefulness are Agent/Human judgments. A fixed wording token is neither required evidence nor acceptance.

Only physical inability to evaluate is `BLOCKED / 1`: the requested onboarding root is missing/unreadable, an unsafe local path escapes the root or crosses an unsafe symlink, or source/render authority is mechanically ambiguous. A missing file, Diagram ID, section locator, evidence reference, source/render member, digest mismatch, recorded non-current status, or unresolved placeholder remains visible as a `CHANGED` finding when safe enumeration is possible. The Agent repairs derived evidence inside existing authorization or routes semantic uncertainty to the existing Onboarding Human Review; it never treats exit `0` as authorization.
...
Working Archify renders are review aids and are not written into the durable manifest. A separately confirmed durable pair records the typed JSON source, derived render, both SHA-256 values, exact generator, `validate=pass; check=pass`, and `Status: current` in `## Diagram Artifact Manifest`. The Onboarding validator reports missing/stale pair members, digests, status, and symbol/config evidence as `CHANGED` when the owning root and pair authority remain safely enumerable; duplicate IDs, one source claimed by conflicting renders, source/render identity, path escape, or unsafe symlink remains `BLOCKED`. If Archify is unjustified, declined, unsupported, unavailable after recommendation, or failed, use embedded Mermaid/ASCII; absence never blocks Onboarding completeness.
