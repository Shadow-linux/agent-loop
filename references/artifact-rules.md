...
The Package Digest is `raw-v1`: SHA-256 over sorted `<Feature-relative-path><TAB>sha256:<raw-file-digest>` rows with a final newline. It inventories every current file under triggered `tasks/`, `tests/`, `plans/`, and `contracts/` directories plus optional `context.md` / `contracts.md`; a self-reported subset is invalid. Stable Digest records an explicit algorithm. New evidence uses `review-definition-v2`, excludes rotatable `plan.md` and `plans/*`, hashes definition-only files as raw bytes, and applies the section-aware task/test projections below. `scripts/check-feature-review.py --mode digest` computes copyable evidence through the same implementation without writing files. `review` and package-only `start` require the raw full Package Digest; active `execute` permits a newly rotated detailed Plan but requires the complete versioned Stable Digest, accepted Tasks that actually exist and are classified Agent-ready, and Plan/No-Plan evidence matching an Active Plan Scope inside the accepted task set.

`review-definition-v2` normalizes only published runtime ledger values: task checkboxes and task `Status` / `Review` / `Drift`; task-detail top-level `Updated` / `Status` plus Task Done Gate checkboxes and `Evidence` / `Review` / `Drift`; test-ledger top-level `Updated` / `Status`, Design Slice matrix `Status`, and Bug matrix `Result` / `Evidence Link`; test-detail top-level `Updated` / `Status`. It continues to hash IDs, order, titles, mappings, Mode, dependencies, gates, acceptance, verification, declared evidence location, commands, assertions, expected evidence, environment, safety, cleanup, interface, risk, and rollback meaning. Projected Markdown is decoded as UTF-8 (optional BOM), canonicalized to LF with one final newline, and parsed section-aware with a whitelist. Malformed recognized rows or ambiguity fail closed; projection is not a broad line deletion.

Explicit `raw-v1` remains readable as a legacy stable algorithm. A missing or unknown algorithm fails closed. An unchanged legacy baseline may be re-recorded as v2 only after Human review of the exact algorithm and digest. A mismatching raw legacy baseline returns to Gate 2 unless preserved evidence proves the exact runtime-only delta and the Human accepts migration; there is no automatic migration, overwrite, force, or bypass.
