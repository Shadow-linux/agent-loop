...
Direct Edit creates no artifact. Do not add a Direct Edit row to this ownership table, create `.agent-loop/direct-edits/`, reuse a Lightweight Execution Card, append a per-edit Feature notes row, or write project memory merely to preserve the action. Its response-local result, final attributed diff, and minimum matched check are sufficient only for the trivial edit; an edit requiring durable recovery or a stable future fact exits Direct Edit before broader work.
