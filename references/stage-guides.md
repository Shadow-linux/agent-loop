...
- requirement checklist
- Core Invariants table: one row per stable invariant with its Spec/Requirement/ADR source and the case that verifies it; depth follows the recorded Feature Verification Profile tier
- Test Oracle table: expected result per case with its oracle source and an oracle quality check confirming each case maps to an invariant or acceptance item and no pass-green-but-core-unverified gap remains
- functional cases
- module tests
- API tests where applicable
- Web E2E/browser cases where applicable, after loading `e2e-discovery.md`
- regression tests scoped to the Profile tier: affected existing checks for `focused`, impacted regression for `full`, extended regression for `high-assurance`; Existing Test Obligations apply at every tier
- manual verification
- commands
