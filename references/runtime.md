...
Feature `notes.md` must also persist the Gate decisions and comparison baseline: `Gate 1 Decision` / `Gate 1 Spec Digest`; `Gate 2 Decision` / `Gate 2 Reviewed At`; `Gate 2 Package Files` / `Gate 2 Package Digest`; `Gate 2 Stable Files` / `Gate 2 Stable Digest Algorithm` / `Gate 2 Stable Digest`; `Gate 2 Agent-ready Tasks`; `Active Plan Scope`; `Gate 2 Plan Evidence`; and `Feature Auto-Loop` enabled/disabled. Package Digest is `raw-v1` and covers the complete reviewed package, including every current artifact under triggered `tasks/`, `tests/`, `plans/`, and `contracts/` directories and the initial `plan.md` when Plan Gate requires one. New Stable Digest evidence uses `review-definition-v2`: it covers `spec.md`, `tasks.md`, `tests.md`, optional `context.md`, contracts, task/test details, and every other non-rotatable reviewed package file, but excludes rotatable `plan.md` and `plans/*`; definition-only files remain raw, while section-aware task/test projections normalize only published runtime ledger/result fields and protect all IDs, mappings, boundaries, gates, acceptance, verification, commands, assertions, risk, interface, and rollback meaning. Plan Evidence is `plan.md`, `plans/<detail>.md`, or `no-plan:<accepted task ID>` and must match Active Plan Scope. Use `scripts/check-feature-review.py --mode digest` to calculate evidence read-only and `review | start | execute` to fail closed on incomplete inventory, missing/unknown algorithm, unsafe projection, digest drift, invalid decision/Auto-Loop pairing, non-Agent-ready task claims, or Plan scope outside the accepted task set.

Explicit `raw-v1` remains legacy-readable. Missing or unknown algorithms fail closed. Re-recording an unchanged raw baseline as `review-definition-v2` requires Human review of the new algorithm and digest; a mismatching raw baseline cannot migrate silently and returns to Gate 2 unless exact preserved runtime-only evidence plus Human acceptance proves the migration. The checker never overwrites evidence and exposes no force/bypass.
...
For multiple accepted Agent-ready tasks, Feature Auto-Loop may rotate `plan.md` and Active Plan Scope to the next Gate 2 task or story without repeating Gate 2 only when `review-definition-v2` still matches after normalizing its exact runtime fields, task/test definitions and ordering are unchanged, the new task/story Plan passes Plan Gate and Analyze Consistency, and interfaces/risk/rollback/verification obligations remain within the reviewed package. Completing a task may update only the published task/test runtime ledger values without invalidating stable definition evidence. A task Plan ID must be one accepted Agent-ready task. A story Plan must explicitly list only accepted Agent-ready Tasks in `Included Tasks`, and each included Task must map to that Story in `tasks.md`. Run `python3 <skill-root>/scripts/check-feature-review.py --mode execute <feature-dir>` before execution; on Windows use `py -3`. A new task ID, an unaccepted or story-mismatched task inside a story Plan, any non-whitelisted stable-file change, changed boundary/order, or material risk/interface/rollback/verification change returns to Gate 2.
