...
- use Direct Edit Fast Path for a genuinely trivial deterministic ordinary non-Bug edit with no persistent artifact or new test, while still proving the final diff with one minimum matched check
...
- execute an initial Feature task/story or explicit Bug repair with TDD and verification
...
An explicit request for a safe one-off edit enters Direct Edit Assessment before persistent Lightweight Change. A genuinely trivial deterministic edit may proceed with no Feature, Change card, Plan, No-Plan Decision, new test, notes row, or memory record only when every Direct Edit condition is true; it still requires an exact final diff and one minimum artifact-matched check. Work needing durable control uses Lightweight Change, and any Feature/Bug hard trigger keeps its owning route.

First version excludes: multiplayer workflow, roadmap graph, roadmap adapter, tdd-guard, complex ADR system, automatic or unscoped global skill installation, automatic directory-level `AGENTS.md` generation without human confirmation, and automatic commit/PR/merge/release/publish/seal without human confirmation. A visual adapter may be installed only after a separate, exact Installation Authorization that discloses source, revision, command, target, effects, doctor check, and fallback.
...
references/direct-edit-fast-path.md genuinely trivial deterministic edits, final-only tuning proof, zero-artifact boundary, escalation, and full-run interaction
...
3e. For an actionable ordinary non-Bug change that may be bounded, run Direct Edit Assessment before persistent Lightweight Change. When every Direct Edit condition is true, disclose and perform the exact bounded edit, inspect its final diff, run one minimum artifact-matched check, and create no Agent Loop artifact or new test. When persistence/control is needed, run Lightweight Change Assessment and create one parser-valid card under the single accepted memory root at `changes/YYYY-MM/YYYY-MM-DD-<topic>.md` before the first target write. When a Feature trigger applies, use the existing Feature path. When uncertain, stop and ask the human with options, one Agent recommendation, evidence, and zero writes before the answer.
...
14a. For `feature-archive-maintenance`, load `references/artifact-rules.md`, `references/stage-guides.md`, `references/human-review-summary.md`, and `references/feature-follow-up.md`. Feature ID is stable while location changes and root `features/archive.md` locates archived/rehydrated history. The scan is read-only, never follows ordinary symlinks, reports uncertain-reference facts, and excludes a symlinked Feature entry from ordinary directory moves without authorizing or rejecting the operation. A verified internal memory-root alias retains logical `.agent-loop/...` or legacy paths and contributes target evidence to the plan hash. The Agent reviews coverage, conflicts, risk, and recovery, then recommends the existing expected plan SHA-256 Batch Human Gate or asks one blocking question. Apply rechecks real Feature move paths before creating a transaction and retains the transaction journal, project/plan confinement, post-check, and restore. Rehydrate before reopened execution; auto modes never authorize either operation.
14b. Load `references/direct-edit-fast-path.md` before creating a Lightweight Change card for an already-decided, genuinely trivial deterministic ordinary non-Bug edit. Explicit Bug Management wins; active Feature ownership keeps an eligible Direct Edit inside its current write grant. Direct Edit creates no stage, intent, status, mode, artifact, Plan/No-Plan Decision, test, notes row, scanner entry, or memory write; exact final diff plus one minimum matched check remains mandatory.
14c. Load `references/lightweight-change-lane.md` when Direct Edit is ineligible because bounded persistence/control is needed but no Feature hard trigger applies. Explicit Bug Management and active Feature ownership take precedence. The lane is an internal route, creates no canonical stage, persists exactly one monthly Change card with an adaptive Plan, runs the read-only pending-memory scanner, and stops before writes when uncertain or before broader edits on scope expansion.
...
- Before Feature construction for a bounded ordinary non-Bug change, run Direct Edit Assessment. Genuinely trivial deterministic work uses zero-artifact Direct Edit; work needing persistence/control enters Lightweight Change and persists one card before target writes; Feature hard triggers use Feature construction; uncertainty performs zero writes and returns one recommended human choice.
...
- Direct Edit never creates a Plan or No-Plan Decision and never invokes Plan/TDD helpers. It applies only the disclosed bounded edit, then performs one final diff inspection and minimum matched check; same-scope Feature tuning waits until the Human chooses the final value. If a new test is the only reliable proof, or durable recovery/control is needed, exit Direct Edit.
- The Lightweight card Plan is always required but adaptive. A clearly eligible Lightweight Change applies the disclosed bounded change first, then runs fresh targeted verification, affected existing checks, diff/scope/risk/rollback review, and records a specific Regression Test Advisory or not-needed reason. Card execution never grants Git, release, production, external, or Bug lifecycle authority.
...
- Task Done Gate: mark a task `done` only after implementation is complete, Required Verification and every Existing Test Obligation or applicable Human-approved substitute have run fresh, evidence is recorded in `notes.md`, lightweight Spec Review is recorded, Standards Review is recorded when triggered, drift decision is recorded, and `tasks.md` links or names the evidence. An unaccepted Additional Regression Test Advisory does not by itself block completion.
...
- Feature Spec enters Brainstorm / Clarify only when real feature-local scope, acceptance, or implementation-boundary uncertainty remains after loading the accepted Product Slice and applicable ADRs. A clear Feature skips brainstorming; helper availability alone is not a trigger, and brainstorming cannot redefine accepted product or ADR meaning.
...
- Human-Guided Bug Management is an internal method of Feature Follow-up / Flow-back, not a canonical stage or message intent. Before creating, updating, or reopening a stable Bug Record, scan all Bug Index metadata for duplicate/reopen identity, then scan the default 90-day Feature metadata window with evidence-ranked deep read and evidence-driven extension beyond 90 days. A matched closed Bug remains closed until its separate Bug Reopen Gate is explicitly accepted. Only then append its reopen lifecycle evidence and recommend a newly gated Resolution Path.
...
- Auto modes stop before relying on Feature Context when `CHANGED` lacks a completed Agent assessment/current refresh or when `BLOCKED` reports physical authority failure; they also stop at Human-gated work, unclear decisions, risky changes, failed verification, drift needing approval, unrelated dirty work blocking progress, Bug Resolution Path decisions, Bug close/reopen, Feature create/reopen, Requirement create/lifecycle change, Delivery Contract creation/acceptance/breaking changes, archive/rehydrate apply, directory guidance changes, unapproved subagent dispatch, branch creation, switching, deletion, push, or tag, submit, pause, close, commit, PR, merge, release, publish, or seal.
...
- Normal Feature construction records a Feature Verification Profile (`focused | full | high-assurance`) with rationale, hard floors, and escalation triggers during package preparation after Gate 1; Gate 2 presents it, execution only auto-escalates with recorded evidence, and a post-Gate 2 downgrade requires presented changed-risk evidence plus recorded Human acceptance. Hard floors are never lowered by human urgency, acceptance, or responsibility claims; only a changed floor-category fact recomputes them. The Profile is an internal verification strategy, not a stage, status, Mode, or Gate, and never assigns a tier to Direct Edit, Lightweight Change, or Review Repair, though hard-floor contact exits a light path and escalates the owning Feature.
- Targeted/affected evidence is the default. Before a newly proposed repository-wide suite, all-packages/E2E/environment run, or six-domain full validation, present the exact command set, repository/target, branch/full HEAD, relevant input/environment, expected cost/effects, supported claim, and expiry. One Human confirmation authorizes one unchanged execution; changed command/target/HEAD/input/environment or a manual rerun requires fresh confirmation. An explicit exact Human instruction or a Gate 2 row that visibly accepts the same exact run already satisfies the Gate and must not be prompted twice. A Verification Profile label alone is not execution permission.
...
- Normal Submit readiness requires fresh verification, drift check, diff review, human confirmation, and a recorded submit note. An explicit commit or commit-and-push request may instead use the Full-Worktree Git Fast Path: one lightweight confirmation of the entire-worktree summary and Agent-proposed commit message, no tests or code-quality Review merely for Git, then `git add -A`; this packaging path cannot make a completion or release-readiness claim. Commit never implies tests. A Push Gate discloses predictable automatic full CI; approval covers that CI without a duplicate local full run, while a manual CI rerun needs a new exact confirmation. Declining Release-required full validation keeps Release/readiness blocked.
...
- Initial Feature and explicit Bug implementation: use a TDD skill if available. Direct Edit Fast Path, Review Repair Fast Path, and clearly eligible Lightweight Change do not invoke it by default.
...
- a newly proposed full-test/full-validation execution lacks its exact current Human confirmation; its command, target, branch/full HEAD, relevant input/environment, or Gate changed; or a manual rerun is about to reuse expired authority
...
- Feature Monthly Archive or rehydrate lacks an exact reviewed plan hash, the Agent cannot establish sufficient reference/risk coverage from advisory findings, encounters a stale plan or incomplete `.archive-txn`, or would be performed by manual directory movement; unsupported/ambiguous findings alone are not a Checker Gate
