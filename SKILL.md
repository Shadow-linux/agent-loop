...
- The card Plan is always required but adaptive. A clearly eligible Lightweight Change applies the disclosed bounded change first, then runs fresh targeted verification, affected existing checks, diff/scope/risk/rollback review, and records a specific Regression Test Advisory or not-needed reason. Card execution never grants Git, release, production, external, or Bug lifecycle authority.
...
- Initial Feature and explicit Bug implementation: use a TDD skill if available. Review Repair Fast Path and clearly eligible Lightweight Change do not invoke it by default.
